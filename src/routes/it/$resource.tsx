...
export const Route = createFileRoute('/it/$resource')({
  beforeLoad: ({ params }) => {
  ...
  head: ({ params }) =>
    resources[params.resource]
      ? pageHead(resources[params.resource]!, 'it')
      : staticPageHead(params.resource, 'it'),
  component: () => {
    const slug = Route.useParams().resource;
    return resources[slug] ? (
      <ResourcePage kind={resources[slug]!} locale="it" />
    ) : (
      <StaticPageContent slug={slug} locale="it" />
    );
  },
});
