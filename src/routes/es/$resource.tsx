...
export const Route = createFileRoute('/es/$resource')({
  beforeLoad: ({ params }) => {
  ...
  head: ({ params }) =>
    resources[params.resource]
      ? pageHead(resources[params.resource]!, 'es')
      : staticPageHead(params.resource, 'es'),
  component: () => {
    const slug = Route.useParams().resource;
    return resources[slug] ? (
      <ResourcePage kind={resources[slug]!} locale="es" />
    ) : (
      <StaticPageContent slug={slug} locale="es" />
    );
  },
});
